...
int MyTabWidget::appendNormalPage(QWidget *page,QString TabText)
{
    if(!page)
        return -1;
    //设置为调用close时释放
    ...
    const int index=addTab(page,TabText);
    //切换为当前新增页
...
void MyTabWidget::initTabBar()
{
    ...
    setTabBar(bar);
    //点击页签上的关闭按钮时，触发信号
//    connect(bar,&QTabBar::tabCloseRequested,this,&MyTabWidget::removeNormalIndex);//不支持删除
    //拖拽到外部-还未释放鼠标
    connect(bar,&MyTabBar::beginDragOut,this,[this,bar](int index){
        ...
        // 根据索引获取当前选项卡的文本
        QString currentTabText = this->tabText(this->currentIndex());
        ...
        connect(drag,&QDrag::destroyed,this,[=]{
            ...
            if(!bar->contentsRect().contains(bar_point)){
                popPage(drag_tab,currentTabText);
            }
        });
...
void MyTabWidget::popPage(QWidget *page,QString TabText)
{
    takeNormalPage(page);
    //这里套一个自定义标题栏的窗口给page
    ...
    connect(pop,&MyTabPopup::dragRelease,this,[=](const QPoint &pos){
        ...
        if(tabBar()->contentsRect().contains(bar_pos))
        {
            ...
            this->appendNormalPage(content,TabText);
            pop->disconnect();
            //关闭的时候会在原来的位置闪一下？
            pop->close();
            //this->activateWindow();
    ...
    pop->setWindowFlag(Qt::WindowCloseButtonHint,false);//去除标题栏中的关闭按钮
    pop->setWindowFlag(Qt::WindowContextHelpButtonHint,false);//去除标题栏中的问号
    pop->show();
    page->show();
    pop->activateWindow();
    pop->setFocus();
